...
private:
	...
	char rxBuff[PACKETSIZE];		// receive packet buffer
...
// if there is a message in the queue, copy it
// into Flock's TX buffer and transmit it. if
// transmission succeeds, dequeue the message.
//
void SRFlock::txDequeue () {

	if (txQueueCount() && connected()) {
		int o= out + 1; if (o >= __FLOCK_TXQDEPTH) o= 0; // the packet
		int n= strlen (txQueue [o]);	// it's length
		if (packetWrite (o, n)) {	// if success,
			out= o;			// remove from queue
			T.setTimer (__FLOCK_PEEPTIMER, peepUpTime);
...
int SRFlock::txQueueCount () {
	...
	int a= in - out - 1; if (a < 0) a += __FLOCK_TXQDEPTH;
	return a;
}

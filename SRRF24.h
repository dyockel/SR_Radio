...
protected:
  ...
  void write_register_bits (uint8_t reg, uint8_t bits);
  void flush_rx (void);
  void flush_tx (void);
  void powerUp (void);
...
// actually power down the chip. called from userland only.
//
...
//
	if (selfAddress) setTXID();			// set ourselves as PTX
...
//
	ce (HIGH);					// begin transition to TX
...
// case means the code is doomed anyway so who cares.) i deemed this
// simpler than queue management or callback schemes.
//
	const int TXTIMEOUT = 77;			// whatever, we're doomed
	uint8_t status = 0;
	uint32_t T= millis() + TXTIMEOUT;
	do {
		status = read_register (STATUS);
		if (status & _BV(TX_DS)) {		// SUCCESS, why wait around
			int obs = read_register (OBSERVE_TX);
			available();			// immediately to RX Mode
			return (obs & 0x0f) + 1;	// number of tries it took
		}
		if (status & _BV(MAX_RT)) break;	// max retries reached, bad
	} while (millis() < T);

// TX failed. flush and return error code.
//
	flush_tx();					// transmit failed, flush
	available();					// immediately to RX mode
	if (status & _BV(MAX_RT)) return -1;		// RX wasn't acknowledged
	return (-1 & ~255) | status;			// < 0, status in lower 8
}
...
int SRRF24::available(void) {
	...
	powerUp();
	if (! (read_register (CONFIG) & _BV(PRIM_RX))) { // switch to RX Mode
		write_register_bits (CONFIG, _BV(PRIM_RX));
		if (selfAddress) setRXID();		// set ourselves as PRX
		ce (HIGH);
// just wait it out	
		delayMicroseconds (150);	// incur RX Settling delay
		recentRPD= 1;				// clear this
	}

// if RX ready now, return most-recent RPD reading, else update it, return false.
//
...
//
	uint8_t d= 32;					// fixed payload size
	if (dynPayload)  {				// if dynamic payloads
		csn (LOW);
		SPI.transfer (R_RX_PL_WID);		// use what the radio sez
		d= SPI.transfer (0xff);
		csn (HIGH);
	}
	if (d < len) len= d;				// fewer in FIFO than buff len

// figure out how many bytes to drop to not overflow the
// buffer
//
	d -= len;
...
//
void SRRF24::setSelfAddress (uint64_t base, uint8_t t, uint8_t r) {

	setFixedAddress (base, base);			// set 64-bit pipe addresses
	setSelfAddress (t, r);				// then store IDs
}

// using current base address, set TX and RX IDs.
//
void SRRF24::setSelfAddress (uint8_t t, uint8_t r) {
...
void SRRF24::setFixedAddress (uint64_t t, uint64_t r) {
	...
	write_register (TX_ADDR,    reinterpret_cast<uint8_t*>(&t), 5);
	write_register (RX_ADDR_P0, reinterpret_cast<uint8_t*>(&t), 5);
	write_register (RX_ADDR_P1, reinterpret_cast<uint8_t*>(&r), 5);
...
// 
void SRRF24::setPALevel (uint8_t n) {

#define RFPOWERBITS (_BV(RF_PWR_LOW)|_BV(RF_PWR_HIGH))

	n &= 3;
	n <<= RF_PWR_LOW;
	uint8_t r= read_register (RF_SETUP) & ~RFPOWERBITS;
	write_register (RF_SETUP, r | n);
}

// set radio data bit rate; 0=250K, 1=1MBPS, 2=2MBPS.
//
void SRRF24::setDataRate (uint8_t n) {

	uint8_t r= read_register (RF_SETUP) & ~(_BV(RF_DR_HIGH) | _BV(RF_DR_LOW));
	switch (n) {
		case 0: r |= _BV(RF_DR_LOW); break;	// set low-rate bit (250K)
		case 1: break;				// both clear (1M)
		case 2: r |= _BV(RF_DR_HIGH); break;	// set high-rate bit (2M)
	}
...
//
void SRRF24::setARD (uint8_t n) {

	n &= 15;
	n <<= 4;					// bits 7:4
	uint8_t r= read_register (SETUP_RETR) & B00001111;
...
// to 15.
//
void SRRF24::setARC (uint8_t n) {

	n &= 15;					// bits 3:0
	uint8_t r= read_register (SETUP_RETR) & B11110000;
	write_register (SETUP_RETR, r | n);
}

// set radio channel.
//
void SRRF24::setChannel (uint8_t channel) {
...
//
uint8_t SRRF24::read_register (uint8_t reg) {
...
//
void SRRF24::write_register (uint8_t reg, const uint8_t* buf, uint8_t len) {
...
void SRRF24::write_register (uint8_t reg, uint8_t value) {
...
//
void SRRF24::write_register_bits (uint8_t reg, uint8_t bits) {

...
private:
	...
	uint8_t chanThresh;			//
	uint8_t badChan[(MAXCHANNEL+1)/8];	// bad channels, 0..127, bitmap
...
int SRFlock::flockToBase () {
	...
	switch (state) {
		...
		//
		case 0:	
			channel= random (minChannel, maxChannel + 1);
			radio.setChannel (channel);
		...
		case 1:
			...
			T.setTimer (RADIOTIMER, random (5, 11));
		...
		// and the other means time to renegotiate.
		//
		case 3:	
			r= getPacket (true);	
			if (r) {
				r= dupCheck (r);	// check for duplicate packet
				copyN (rxBuff, pxBuff, r);// deliver payload
				if (pxBuff[2] == '!') ackPacket (pxBuff[1], false);
			}
			// if quiet too long request acknowledgement from the base
			// to ensure the channel is alive.
			//
...
// upon receipt of any packet for us.
//
...
// for address and general validity. at the moment we only accept
...
//
int SRFlock::getPacket (bool allOK) {
	...
	// can't think of any.
	//
	if (pxBuff[1] != '@') return 0;			// only from: base

	random();					// break the sequence
	T.resetTimer (RXTIMER);				// channel is alive
	T.resetTimer (RXTOTIMER);

	// packets addressed to * are to all, eg. us.
	//
	if (allOK && pxBuff[0] == '*') pxBuff[0]= identity;// to: * is to: us

	if (pxBuff[0] != identity) return 0;		// listen only to: us
	blink (LEDBRIEF);				// got one
	if (r < 3) pxBuff[2]= 0;			// see comment

	// packet to us: down-count channel error counter, which is
	// incremented upon every transmit that requests acknowledge.
	//
	if (ackBalance) --ackBalance;

	return r;					// it's for us.
}
...
uint8_t SRFlock::dupCheck (uint8_t r) {
	...
	int crc= crc8buff (pxBuff, r);			// CRC the packet
...
// generate the CRC8 of n bytes at pointer p. this is the
// Dallas/Maxim CRC8 algorithm, below, looped.
//
uint8_t SRFlock::crc8buff (uint8_t * p, uint8_t n) {
...
//
uint8_t SRFlock::crc8 (uint8_t crc, uint8_t data) {
uint8_t i;
...
TODO: sent channel-change hint packet to *.
...
int SRFlock::baseToFlock () {
	...
	// if no packet, check the ack sent/received counter. if too
	...
	//
	if (rxTimeout && T.timer (RXTIMER)) ackPacket ('*', false);
...
// mark the given channel as bad, select a new one, clear channel
// statistics, and also randomly un-mark a bad channel as good. (inhale)
// this tracks badness in a bitmap, one bit per channel, 0..127 
// channels/0..127 bits.
// 
// channel 0 and 127 not actually used.
//
uint8_t SRFlock::newChan (uint8_t b) {

	// do nothing if not enabled.
	//
	if (! dynamicChannelMapping) return b;
...
bool SRFlock::messageDispatcher (char c, unsigned nnn) {
  ...
  bool ok= true;
	...
	switch (c) {
		...
		case 'd':					break;
		case 'e': 					break;   
		case 'f': setChannel (nnn);			break;    // base only
		case 'g': nextChannel();			break;    // base only
		case 'h': setMinChannel (nnn);			break;    // base only
		case 'i': setMaxChannel (nnn);			break;    // base only
		case 'j': dynamicChannelEnable();		break;    // base only
		case 'k': dynamicChannelDisable();		break;    // base only
		case 'l': setDataRate (nnn);			break;    // base only
